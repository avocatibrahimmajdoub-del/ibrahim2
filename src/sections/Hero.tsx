...
import { Btn, Label, Reveal } from "../components/ui";

function Stat({ value, suffix, label, active, delay }: { value: number; suffix: string; label: string; active: boolean; delay: number }) {
  ...
  return (
    <Reveal delay={delay} className="border-t border-white/12 px-6 py-7 lg:px-8">
      ...
      <p className="mt-3 text-[0.68rem] uppercase tracking-[0.2em] text-white/50">{label}</p>
    </Reveal>
  );
}
...
export function Hero() {
  ...
  return (
    <section id="accueil" className="relative overflow-hidden bg-navy-950">
      <img src={heroBg} alt="" className="absolute inset-0 h-full w-full object-cover opacity-[0.22]" />
      ...
      <div className="relative mx-auto max-w-7xl px-5 pt-16 pb-0 sm:px-8 lg:pt-24">
        <div className="grid items-center gap-14 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="pb-16 lg:pb-24">
            ...
            <Reveal delay={130}>
              <p className="mt-7 max-w-xl text-[0.97rem] leading-relaxed text-white/70">{t.hero.lead}</p>
            </Reveal>
            ...
            <Reveal delay={250}>
              <dl className="mt-12 grid gap-y-6 border-t border-white/12 pt-8 sm:grid-cols-3 sm:gap-x-8">
                {t.hero.credentials.map((c) => (
                  <div key={c.k}>
                    <dt className="text-[0.62rem] uppercase tracking-[0.22em] text-brass-300/85">{c.k}</dt>
                    <dd className="mt-2 text-[0.86rem] leading-snug text-white/80">{c.v}</dd>
                  </div>
                ))}
              </dl>
            </Reveal>
            ...
            <Reveal delay={310}>
              <div className="mt-8 flex flex-wrap items-center gap-6 text-[0.8rem] text-white/60">
            ...
            <div className="relative">
              ...
              <div className="relative overflow-hidden border border-white/12">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-navy-950 via-navy-950/70 to-transparent px-6 pt-16 pb-6">
                  ...
                  <p className="mt-1.5 text-[0.6rem] uppercase tracking-[0.26em] text-brass-300/90">
                    {t.hero.portraitRole}
                  </p>
                </div>
              </div>
              <div className="mt-5 flex items-center gap-4">
                ...
                <span className="max-w-[9rem] text-[0.6rem] uppercase leading-tight tracking-[0.18em] text-white/45">
                  {t.about.badge.label}
                </span>
              </div>
            </div>
