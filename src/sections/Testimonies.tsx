...
import { Btn, Reveal, SectionIntro } from "../components/ui";
    ...
    <>
      <section id="avis" className="bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <SectionIntro
            eyebrow={t.testimonies.eyebrow}
            title={t.testimonies.title}
            titleEm={t.testimonies.titleEm}
          />
            ...
            {t.testimonies.items.map((item, i) => (
              <Reveal key={item.name} delay={i * 70} className="bg-white">
                <figure className="flex h-full flex-col p-7 sm:p-9">
                  ...
                  <blockquote className="mt-5 flex-1 text-[0.95rem] leading-relaxed text-navy-800">
                    “{item.quote}”
                  </blockquote>
                  <figcaption className="mt-7 border-t border-navy-900/10 pt-5">
                    <span className="block text-[0.88rem] font-medium text-navy-900">{item.name}</span>
                    <span className="mt-1 block text-[0.72rem] uppercase tracking-[0.14em] text-navy-500">
                      {item.role}
                    </span>
                  </figcaption>
                </figure>
          ...
          <Reveal delay={120}>
            <p className="mt-6 max-w-3xl text-[0.74rem] leading-relaxed text-navy-500">{t.testimonies.note}</p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-navy-950 py-16 sm:py-20">
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-5 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-5 max-w-xl text-[0.92rem] leading-relaxed text-white/65">{t.ctaBand.text}</p>
          </div>
...
function Faq() {
  ...
  return (
    <div className="border-t border-navy-900/12">
          ...
          <Reveal key={item.q} delay={i * 45}>
            <div className="border-b border-navy-900/12">
              <button
                ...
                className="flex w-full items-start gap-5 py-5 text-left rtl:text-right"
                aria-expanded={isOpen}
              >
                <span className="flex-1">
                  <span
                    className={cn(
                      "block text-[0.98rem] font-medium leading-snug transition-colors duration-300 sm:text-[1.05rem]",
              ...
              >
                <div className="overflow-hidden">
                  <p className="flex gap-3 text-[0.89rem] leading-relaxed text-navy-700">
      ...
      <Reveal delay={180}>
        <p className="mt-6 text-[0.74rem] leading-relaxed text-navy-500">{t.contact.form.note}</p>
      </Reveal>
    </div>
  );
}
